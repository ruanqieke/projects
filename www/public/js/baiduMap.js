...
(function($) {
    $.fn.lockBaiduMap = function(options) {
        ...
        mapShow();
        $('#map').animate({width:1083,height:327})
        obj.fadeOut();
        init();
    };
    var map={city:'北京',address:'北京东城区'};
    $('#map1').lockBaiduMap(map);
    var arr=[{city:'北京',address:'北京东城区'},{city:'太原',address:'小店区'},{city:'苏州',address:'吴中区'}]
    $('.city .city-list').on('click',function () {
        map=arr[$(this).index()];
        $('.city .city-list').find('.city-hover').css({opacity:0});
        $(this).find('.city-hover').css({opacity:1});
        $('#map1').lockBaiduMap(map);
    })
